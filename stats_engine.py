...
def add_normalized_yield(df):
    """Z-score normalize yield within each crop group so different crop scales
    (e.g., sugarcane vs wheat) don't distort cross-group comparisons."""
    df = df.copy()
...
def format_p(p_val):
    """Consistent p-value formatting across all tests."""
    return round(float(p_val), 5) if p_val > 1e-10 else "< 1e-10"

def test_fertilizer_effect(df):
    """Independent t-test: does high fertilizer usage significantly affect
    yield (normalized within crop type)?"""
    ...
    direction_word = "higher" if high_mean > low_mean else "lower"
    return {
        ...
        "direction": f"High fertilizer usage is associated with {direction_word} normalized yield compared to low fertilizer usage.",
        "note": "Yield z-normalized within each crop to control for scale differences across crop types."
    }

def test_state_variation(df):
    """One-way ANOVA: does normalized yield differ significantly across states?"""
    df = add_normalized_yield(df)
    ...
    top_state, bottom_state = state_means.index[0], state_means.index[-1]
    return {
        "test": "One-way ANOVA",
...
def test_rainfall_correlation(df):
    """Pearson correlation: is rainfall correlated with normalized yield?"""
    df = add_normalized_yield(df)
    corr, p_val = stats.pearsonr(df["annual_rainfall"], df["yield_z"])
    direction_word = "positively" if corr > 0 else "negatively"
    return {
        ...
        "p_value": format_p(p_val),
        "significant": bool(p_val < 0.05),
        "direction": f"Rainfall is {direction_word} correlated with normalized yield." if p_val < 0.05 else "No significant correlation was found between rainfall and normalized yield."
    }

def test_pesticide_correlation(df):
    """Pearson correlation: is pesticide usage correlated with normalized yield?"""
    df = add_normalized_yield(df)
    ...
    return {
        ...
        "p_value": format_p(p_val),
        "significant": bool(p_val < 0.05),
        "direction": f"Pesticide usage is {direction_word} correlated with normalized yield." if p_val < 0.05 else "No significant correlation was found between pesticide usage and normalized yield."
    }

def test_yearly_trend(df):
    """Mann-Kendall trend test: has normalized yield changed significantly over years?"""
    df = add_normalized_yield(df)
...
def test_crop_season_independence(df):
    """Chi-square test: are crop type and season independent?"""
